...
```{r}
mod_inc <- glmer(inc_pc/n_plantas ~ hibrido + (1|localidad/hibrido), 
                 weights=n_plantas, 
                 family="binomial", 
                 df)
plot(mod_inc)
...
summary(mod_rinde)
Anova(mod_rinde)
mod_rinde1 <- lm(rinde_bonif ~ hibrido * localidad, data=df)
anova(mod_rinde1, mod_rinde)
```

```{r}
em_rinde_loca <- emmeans(mod_rinde, ~localidad)
...
ggsave(last_plot(), file = "plots/inc_rend.png", w=9, h=4.5)
```
